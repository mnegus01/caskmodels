...
outfilepath = "/global/home/users/mgnegus/CaskModels/char_tests/ADVANTG_tests/"
geomnames = ['WS_problem','beam','maze1','maze2','prob_1','prob_2','prob_4','reactor','therapy-room']

methods = ['cadis','fwcadis']
mcnp_tally = {'cadis':44,'fwcadis':24}
...
if __name__ == '__main__':
    for method in methods:
		...
		inputfilename = infilepath+filename 
                print('Opening file: '+inputfilename)
                templatefile = open(inputfilename,'r')
                ...
                # Update to reflect method and  quadrature type
                changedlist = changeoptions(template,'method',method)
		changedlist = changeoptions(template,'mcnp_input',geomname+"-"+method)
                changedlist = changeoptions(template,'mcnp_tallies',mcnp_tally[method])
		changedlist = changeoptions(template,'denovo_quadrature',quad)
                ...
                for quadO in quadOs:
                    ...
                    for pnO in pnOs:
                        ...
                        runname = filename[:-5]+'_%s_%s%s-%s' %(method[:3],quad,str(quadO),str(pnO))
                
                        outputfilename = outfilepath+'pydriver/'+method+'/'+runname+'.py'
                        if ADVANTGrun == 1:
                            ...
                            outputfile.close()
                            if SLURMrun == 1:
                                runscriptfilename = outfilepath+'runscripts/'+method+'/'+runname+'.sh'
